...
export interface SourceAggregateDto {
  id: number
  name: string
  hostUrl: string
  listsCount: number
...
export interface SourceAggregateResponse {
  sources: SourceAggregateDto[]
  pager: {
    currentPage: number
    pageSize: number
    totalItems: number
...
export function getSources(page = 1, pageSize = 15, search?: string): Promise<SourceAggregateResponse> {
